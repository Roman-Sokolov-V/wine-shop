...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Authenticated users see only their own appointments. "
        "Staff users see all.",
        responses={200: AppointmentSerializer(many=True)},
    ),
    retrieve=extend_schema(
        summary="Retrieve an appointment",
        description="Retrieve a single appointment by ID. Only owner "
        "or staff can access.",
    ...
    create=extend_schema(
        ...
        description=(
            ...
            "If the user's information (email, name, phone) already exists "
            "in the related User instance, "
            "it will automatically be copied into the appointment instance, "
            "regardless of the input.\n\n"
            "If this information is not present in the User instance, it must "
            "be provided when creating the appointment. "
            "In that case, the data will also be saved to the associated User "
            "instance.\n\n"
            "Upon successful creation:\n"
            "- A notification is sent to staff asynchronously.\n"
            "- A scheduled background task is created to automatically "
            "deactivate the appointment 2 hours after its scheduled time."
        ),
    ...
    partial_update=extend_schema(
        ...
        description="Used to update status or other fields partially.Only "
        "owner or staff.",
...
class AppointmentViewSet(
    ...
    def perform_create(self, serializer):
        ...
        1. **Send staff notification** — after saving, an asynchronous task is
        triggered to notify staff via email with the submitted appointment
        details.

        2. **Schedule a status update** — a delayed periodic task is
        registered to automatically mark the appointment as inactive exactly
        2 hours after the scheduled appointment time.

           - If a task with the same name already exists for this appointment,
           it is removed.
           - The task is one-off and uses a `ClockedSchedule` to execute
           precisely at the target time.
        """
        ...
        # Create a delayed task to set the appointment status to inactive
        ...
        PeriodicTask.objects.filter(
            name__startswith=f"set status not active for appointment "
            f"with id: {appointment.id}"
        ).delete()

        PeriodicTask.objects.create(
            name=f"set status not active for appointment "
            f"with id: {appointment.id}",
...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Regular users see only their own forms. "
        "Staff can see all.",
    ...
    create=extend_schema(
        ...
        description=(
            ...
            "If the user's information (email, name, phone) already exists in "
            "the associated User instance, "
            "it will automatically be copied into the form, regardless of the "
            "provided input.\n\n"
            "If this information is missing in the User instance, it must be "
            "explicitly provided in the request. "
            "The submitted values will then also be saved to the User "
            "instance.\n\n"
            "Upon successful creation:\n"
            "- An asynchronous task is triggered to send a notification email "
            "to staff containing the form details."
        ),
