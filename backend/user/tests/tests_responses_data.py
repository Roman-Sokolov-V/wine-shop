...
class LoginTestCase(TestCase):
    ...
    def test_register_user(self):
        ...
        self.assertEqual(response.json()["is_staff"], False), "Expected False, if even input True"
        self.assertEqual(response.json()["is_active"], True), "Expected True, if even input False"
        self.assertEqual(response.json()["is_superuser"], False), "Expected False, if even input True"
        self.assertNotEqual(response.json()["date_joined"], data["date_joined"]), "Expected auto_now_add time, not inputted"
        self.assertEqual(response.json()["favorites"], []), "Expected empty list"


    def test_login(self):
        response = self.client.post(reverse("user:login"), data={"email": self.db_user.email, "password": self.password})
        self.assertEqual(
            status.HTTP_200_OK,
            response.status_code
        )
    ...
    def test_me(self):
        ...
        self.assertEqual(response.json()["is_staff"], self.data["is_staff"])
        self.assertEqual(response.json()["is_active"], self.data["is_active"])
        self.assertEqual(response.json()["is_superuser"],
